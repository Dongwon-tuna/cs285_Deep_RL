...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        for i , model in enumerate(mb_agent.dynamics_models) :
            predicted_ob = mb_agent.get_dynamics_predictions(i, ob.reshape(1, -1), ac.reshape(1, -1)) # 1,obdim 처럼 배치형태로 바꿨음 obs acs를 넣는 구조라서.
            next_obs_list.append(predicted_ob)

        next_obs_array = np.array(next_obs_list)
        next_ob = np.mean(next_obs_array, axis=0).squeeze(0) # axis0은 무조건 첫번째것

        rew, _ = env.get_reward(next_ob.reshape(1, -1), ac.reshape(1, -1))# 차원 문제가 종종 발생하니 항상 고려하자
        rew = rew.squeeze(0) 
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i, model in enumerate(mb_agent.dynamics_models):
                ...
                ensemble_loss = mb_agent.update(i, obs= batch["observations"], acs= batch["actions"],next_obs= batch["next_observations"])
                step_losses.append(ensemble_loss)
            ...
            all_losses.append(np.mean(step_losses)) # 여러 앙상블을 봐서 loss를 평균내서  보는거임
