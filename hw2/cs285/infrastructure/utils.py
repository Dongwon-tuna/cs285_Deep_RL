...
def sample_trajectory(
    ...
    while True:
        ...
        ac: np.ndarray = policy.get_action(ob)# None

        # TODO: use that action to take a step in the environment
        next_ob, rew, done, _ =  env.step(ac) #None, None, None, None
        # env.step(ac)를 사용해서 이 환경의 리워드 계산로직을 가져다가 쓰는거임. 직접 우리가
        #정책을 여기선 설계해주진 않음.
...
# 정책에 따라서 여러개의 trajectory 구성해서 batch 를 구성하는 함수. 근데 여기선  
# sample_trajecty()를  min_timesteps_per_batch만큼 trajectory 수집
