...
class RNDAgent(DQNAgent):
    def __init__(
        ...
        **kwargs
        ...
        self.rnd_target_net.apply(init_network) #이 타겟 네트워크는 고정점이다. 그냥 새로운 정도를 비교하는 척도가 되기에 초기화해서 고정시킴
    ...
    def update_rnd(self, obs: torch.Tensor) -> torch.Tensor: # 타겟 네트워크와 로스를 줄여가면서 출력값을 모방한다. 
        """
        Update the RND network using the observations.
        """
        # TODO(student): update the RND network   #시간이 지날수록 예측 네트워크가 고정된 타겟 네트워크의 출력값을 정확하게 모방하게 되므로, 
        # 오차가 점차 줄어들어 에이전트가 모든 상태를 골고루 탐색하도록 유도하는 것
        loss = torch.norm(self.rnd_net(obs) - self.rnd_target_net(obs), dim=-1).mean()
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            rnd_error = torch.norm(self.rnd_net(next_observations) - self.rnd_target_net(next_observations), dim=-1)
            assert rnd_error.shape == rewards.shape
            rewards = rewards +  self.rnd_weight * rnd_error

        metrics = super().update(observations, actions, rewards, next_observations, dones, step) 
        # 부모인 dqn의 update를 진행하겠다. 즉, 타겟과 현재
        #차이를 기반으로 리워드 항을 재정의 하고, q네트워크를 다시 학습하는것임!!
