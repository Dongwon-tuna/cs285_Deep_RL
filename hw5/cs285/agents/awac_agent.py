...
class AWACAgent(DQNAgent):
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            next_qs = next_qa_values.gather(dim=1, index=next_action).squeeze(1)
        ...
        q_values = qa_values.gather(1, actions.unsqueeze(1)).squeeze(1)# 모든 q값들중에 우리의 action에 대한 q값만 뽑아옴
    ...
    def compute_advantage(
        ...
        action_probs = action_dist.probs #q값의 텐서와 곱해주기위하여 액션확률분포를 텐서로 바꾸어줌
    
        values = torch.sum(qa_values * action_probs, dim=1) 
    ...
    def update_actor(
        ...
        loss = -torch.mean(log_probs * weights)
