...
class DQNAgent(nn.Module):
    ...
    #     if random.random() < epsilon :# Exploration and greedy action
    #         action = torch.randint(0, self.num_actions, (1, ))


    #     else :
    #         ciritc_values = self.critic(observation)
    #         action = torch.argmax(ciritc_values, dim=1)
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                next_q_online = self.critic(next_obs)
        ...
        q_values = qa_values.gather(1, action.unsqueeze(1)).squeeze(1)# 모든 q값들중에 우리의 action에 대한 q값만 뽑아옴
